...
def main(questions_path, output_path):
    ...
    for q in tqdm(questions, desc="Processando questões"):
        ...
        ).hexdigest()  # mesmo valor usado no embed_chunks_to_qdrant.py
        ...
        search_result = client.query_points(
            ...
            query_filter=Filter(
                must=[
                    FieldCondition(
                        key="context_id", match=MatchValue(value=context_hash)
                    )
                ]
            ),
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Caminho para o arquivo dev.json",
    )
    parser.add_argument(
        "--output_path",
        type=str,
        required=True,
        help="Arquivo JSON de saída com golden chunks",
    )

...
def generate_code(pipe, prompt_template, question, chunk):
    ...
    output = pipe(
        compiled_prompt, max_new_tokens=200, do_sample=False, return_full_text=False
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to model (base or LoRA fine-tuned)",
    )
    parser.add_argument(
        "--dataset_path", type=str, required=True, help="Path to dataset JSON file"
    )
    parser.add_argument(
        "--output_path", type=str, required=True, help="Path to save generations"
    )
    parser.add_argument(
